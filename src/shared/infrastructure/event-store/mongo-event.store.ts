...
import { EventStore } from "src/shared/application/ports/even-store";
...
export class MongoEventStore implements EventStore {
  ...
  async persist(
    ...
    try {
      ...
      this.logger.log("Events inserted successfully to the event store");
    } catch (error) {
      await session.abortTransaction();

      const UNIQUE_CONSTRAINT_ERROR_CODE = 11000;
      if (error?.code === UNIQUE_CONSTRAINT_ERROR_CODE) {
        this.logger.warn("Events could not be persisted. Aggregate is stale");
  ...
  //     this.eventDeserializer.deserialize(
  //       event.toJSON() as unknown as SerializableEvent,
  //     ),
  //   );
  // }
  ...
  async getEventsByStreamId(streamId: string): Promise<SerializableEvent[]> {
    ...
    return events.map((event) => {
      ...
      if (this.isSerializableEvent(json)) {
        return this.eventDeserializer.deserialize(json);
      } else {
  ...
  isSerializableEvent(obj: any): obj is SerializableEvent {
    return (
